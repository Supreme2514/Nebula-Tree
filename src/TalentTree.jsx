...
export default function TalentTree() {
  ...
  function cycleZoom() {
    ...
    // so its center in viewBox-space is always (540, 620).
    const contentX = (540 - view.x) / scale;
    const contentY = (620 - view.y) / scale;
    setView({ x: 540 - newScale * contentX, y: 620 - newScale * contentY });
    setZoomIdx(nextIdx);
  }
  ...
  const lines = useMemo(() => {
    ...
    NODES.forEach((n) => {
      (n.act || []).forEach((packed) => {
        ...
        // The child is fully maxed but THIS particular alternative wasn't
        // the one that satisfied it - worth calling out with a slow pulse,
        // since it's an interesting "other route" for an already-completed
        // node, distinct from a plain unused option on a partially-filled one.
        const isMaxedUnusedAlt = pulseEnabled && lit && !isActivePath && cur >= n.maxRank;
        ...
        if (activeLinkHref && inactiveLinkHref) {
          ...
          out.push(
            <image
              key={`${target.id}-${n.id}`}
              className={isMaxedUnusedAlt ? "pulse-red-link" : undefined}
              ...
              opacity={isMaxedUnusedAlt ? undefined : isActivePath ? 1 : lit ? 0.6 : 0.4}
        ...
        >
          {CLASSES.map((c, i) => {
            ...
            return (
              ...
              >
                {iconSrc && <img src={iconSrc} alt="" width={20} height={20} style={{ borderRadius: 4 }} />}
                {c.name}
              </button>
            );
          })}
        ...
        >
          ...
          <div
            style={{
              ...
              padding: "5px 10px",
              borderRadius: 6,
              border: spent > pointPool ? "1px solid #e05a4e" : "1px solid #33291f",
              display: "flex",
              alignItems: "center",
              gap: 6,
            }}
          ...
          {(() => {
            ...
            return (
              <>
                {activeGuide && (
                  <button
                    onClick={() => applyGuide(activeGuide)}
                    style={{
                      padding: "6px 12px",
                      borderRadius: 6,
                      border: `1px solid ${GOLD}`,
          ...
          <button
            ...
            style={{
              padding: "6px 12px",
              borderRadius: 6,
              border: `1px solid ${showSummary ? BRONZE : "#3a322b"}`,
              background: showSummary ? BRONZE + "22" : "transparent",
              color: showSummary ? BRIGHT : MUTED,
              fontSize: 12,
              cursor: "pointer",
            }}
          >
          ...
          >
            ...
            {assetHref("nebula-detail-divider") && (
              <img
                ...
                alt=""
                style={{ width: "100%", height: "auto", marginBottom: 16, display: "block" }}
              />
            )}
